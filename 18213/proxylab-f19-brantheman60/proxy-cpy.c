...
void serve(int clientfd)
{
    ...
    //if caching, pass into cache to immediately respond!
    if(caching)
    {
        pthread_mutex_lock(&mutex);
        Cache_check(cache);

        /***Need to let request 2 be able to get result from request 1 if
            they will have the same node***/
    ...
    if(caching && sum <= MAX_OBJECT_SIZE)
    {
        ...
        while(cache->load + sum > MAX_CACHE_SIZE)
            evict_node(cache);
        enter_node(cache, node);
