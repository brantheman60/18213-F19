...
cache_t *req_cache; //storage of pending client requests
...
int parse_uri(char *uri, char *hostname, char *pathname, int *port)
{
    ...
    char *token;
    //if((token = strchr(hostname, ':')) != NULL) //port number is included
...
void serve(int clientfd)
{
    ...
    if(caching)
    {
        ...
        Cache_check(cache);
        //hold off on future requests until first request is done
        //this enables request >=2 to use node from request 1
        while(find_node(req_cache, hostname, pathname, port))
        {
            ;
        }
        ...
        else
        {
            ...
            cnode_t *tmp = create_node(hostname, pathname, port,
                                       NULL, 0);
            enter_node(req_cache, tmp);
        }
    ...
    {
        pthread_mutex_lock(&mut);
        cnode_t *tmp = find_node(req_cache, hostname, pathname, port);
        printf("TMP hostname = %s\n", tmp->host); //!!!!!
        printf("TMP pathname = %s\n", tmp->path); //!!!!!
        printf("TMP port = %d\n", tmp->port); //!!!!!
        delete_node(req_cache, tmp); //!!!!!
        pthread_mutex_unlock(&mut);
    }
...
int main(int argc, char** argv) {
    ...
    cache_init(cache);
    req_cache = Malloc(sizeof(cache_t));
    cache_init(req_cache);
