...
{
    ST_NORMAL, // 0: normal command w/o file redirection
    ST_INFILE, // 1: processing infile token
    ST_OUTFILE // 2: processing outfile token
} parse_state;
...
parseline_return parseline(const char *cmdline,
    ...
    const char delims[] = " \t\r\n";    // argument delimiters (white-space)
                                        // separate arguments in command line w/
                                        // space, tab, carriage return, or new line
    ...
    while (buf < endbuf) {
        ...
        buf += strspn(buf, delims); //advance to next argument
        if (buf >= endbuf) break;
        ...
        if (*buf == '<') {
            ...
            buf++;
            next = strchr(buf, *(buf - 1)); //next becomes pointer to next '/"
        } else {
            /* Find next delimiter */
            next = buf + strcspn(buf, delims); //pointer to next non-delimiter argument
        }
    ...
    // Handling build-in functions specific for this shell
...
static void check_blocked(void) {
    ...
    int res;
    if ((res = sigismember(&currmask, SIGCHLD)) <= 0) {
        if (res < 0) {
            sio_eprintf("Sigismember error\n");
            _exit(1);
        }
        sio_eprintf("WARNING: SIGCHLD not blocked\n");
    }
    if ((res = sigismember(&currmask, SIGINT)) <= 0) {
        if (res < 0) {
            sio_eprintf("Sigismember error\n");
            _exit(1);
        }
        sio_eprintf("WARNING: SIGINT not blocked\n");
    }
    if ((res = sigismember(&currmask, SIGTSTP)) <= 0) {
        if (res < 0) {
            sio_eprintf("Sigismember error\n");
            _exit(1);
        }
        sio_eprintf("WARNING: SIGTSTP not blocked\n");
    }
}
