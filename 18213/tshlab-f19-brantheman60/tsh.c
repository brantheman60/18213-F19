...
void dup2IO(char *infile, char *outfile);
...
void eval(const char *cmdline) {
  ...
  case BUILTIN_NONE:
    ...
    if (pid == 0) {
      ...
      dup2IO(token.infile, token.outfile);
...
/*
 * dup2IO: processes infile and outfile, then changes the file
 *         descriptors
 */
void dup2IO(char *infile, char *outfile) {
  int oldfd;

  if (outfile != NULL) // call outfile
  {
    // allow owner to read/write later
    oldfd = open(outfile, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    dup2(oldfd, STDOUT_FILENO);
    close(oldfd);
  }
  if (infile != NULL) // call infile
  {
    oldfd = open(infile, O_RDONLY, 0);
    if (oldfd < 0) {
