 ...
 *
 *  Segregated List Tips:
 *  4 linked lists:
 *  - 0 to 31 bytes   (if h>>5)
 *  - 32 to 1023 bytes (if h>>10)
 *  - 1024 to 1048575 (if h>>20)
 *  - 1048576 to max bytes (2 GB? = 2.05E9)
 *
 *  ************************************************************************  *
 *  ** ADVICE FOR STUDENTS. **                                                *
...
static const size_t chunksize = (1 << 12); // BW: previously 1<<16

// Allow chunksize to change throughout the program
static size_t new_chunksize = (1 << 16);
...
#define SIZE0 16 // max size of block in free list 0
#define SIZE1 32 // max size of block in free list 0
#define SIZE2 64 // max size of block in free list 1
#define SIZE3 1024 // max size of block in free list 1
#define SIZE4 16384 // max size of block in free list 1
//#define SIZE4 65536  //max size of block in free list 2
//#define SIZE5 4194304  //max size of block in free list 3
...
static void insert_in_free(block_t *block) {
  /*
  if (head_of_free == NULL) {
    head_of_free = block;
    set_next_ptr(head_of_free, NULL);
...
void *malloc(size_t size) {
  ...
  if (block == NULL) {
    ...
    // if(extendsize == asize) new_chunksize <<= 1;
 ...
 */
static block_t *coalesce_block(block_t *block) { //!!!!!not ready for seg list
  ...
  bool prev_alloc =
      extract_alloc(*find_prev_footer(block)); // why use extract alloc?
  ...
  if (prev_alloc && next_alloc) // Case 1
  {
    ...
    /*
    if (block == heap_start) {
      if (block != heads_of_free[index]) // block is not head of free list
        insert_in_free(block); // is doing nothing okay?
    } else {
  ...
  } else if (!prev_alloc && next_alloc) // Case 3
  {
    ...
    block = block_prev;
  } else if (!prev_alloc && !next_alloc) // Case 4: failed!
  {
    /*
...
static block_t *find_fit(size_t asize) {
  ...
  while (true) {
    ...
    // look for an exact size if possible
...
bool mm_checkheap(int line) {
  ...
  for (block = heap_start; get_size(block) > 0; block = find_next(block)) {
    ...
    // Check that if free block, it is in one of the free lists
    if (!halloc) {
      int index = choose_list(block);
