...
static void *get_next_ptr(block_t *block) {
  // pointer specified at the "next" line
...
static void *get_prev_ptr(block_t *block) {
  // pointer specified at the "prev" line
  // dbg_requires(!get_alloc(block));
  void *bp = header_to_payload(block);
  // return *(char **)(bp);
  // printf("Prev pointer is %p\n", (char *)bp + wsize);
...
static void set_next_ptr(block_t *block, block_t *next_free) {
  // changes the value at get_next_ptr to next_free
  // dbg_requires(!get_alloc(block));
  void *bp = header_to_payload(block);
  *(void **)bp = next_free;
  // printf("header_to_payload is now %p", get_next_ptr(block));
}

static void set_prev_ptr(block_t *block, block_t *prev_free) {
  // word_t node is the pointer to the previous free block
  // printf("Block = %p", block);
  // printf("get_size(block) = %ld", get_size(block)); // SOME PROBLEM HERE!!!
  // dbg_requires(!get_alloc(block));
  void *bp = header_to_payload(block);
...
static void insert_in_free(block_t *block) {
  // makes block the head of the free list
  // printf("block: %p", block);
  // printf("head_of_free: %p", head_of_free);
...
static void remove_from_free(block_t *block) {
  // take out block, and link the previous and next blocks
  printf("Removing from free\n");
...
static void print_free() {
  ...
  printf("Beginning to print free list");
  for (block = head_of_free; block != NULL && get_alloc((block_t *)block) == 0;
...
bool mm_init(void) {
  ...
  printf("Done creating initial heap\n\n");
  return true;
}
...
void *malloc(size_t size) {
  ...
  // Try to split the block if too large AND deal with the free list
...
bool mm_checkheap(int line) {
  ...
  // Check that in every block, allocation bits are correct ????
