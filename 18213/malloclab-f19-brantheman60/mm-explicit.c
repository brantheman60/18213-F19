...
static const size_t chunksize = (1 << 16); // BW: previously 1<<12
...
static void remove_from_free(block_t *block) {
  ...
  // if(block == NULL) printf("OMG REMOVING NULL FROM FREE LIST!\n");
  dbg_assert(block != NULL);

  if (prev == NULL && next == NULL) // Case 1: only block in free list
  {
    head_of_free = NULL; // works
  } else if (prev == NULL &&
             next != NULL) // Case 2: head_of_free and >=1 in free list
  {
    head_of_free = next;
    set_prev_ptr(next, NULL);
  } else if (prev != NULL && next == NULL) // Case 3: last in list
  {
    set_next_ptr(prev, NULL); // works
...
static block_t *coalesce_block(block_t *block) {
  ...
  if (prev_alloc && next_alloc) // Case 1
  {
    ...
    if (block == heap_start) {
      if (block != head_of_free) // block is not head of free list
  ...
  } else if (!prev_alloc && next_alloc) // Case 3: failed!
  {
...
static block_t *find_fit(size_t asize) {
  ...
  */
  ...
  for (block = head_of_free; block != NULL; block = get_next_ptr(block)) {
    ...
    if (asize <= block_size)
    {
      if (block_size <= 2*asize)  // asize <= block_size <= 4*asize
         return block;
      else safety_block = block;
    }
  }
  return safety_block;
  */
