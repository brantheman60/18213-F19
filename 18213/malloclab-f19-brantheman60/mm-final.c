...
static const size_t chunksize = (1 << 9); //15 //12

// Allow chunksize to change throughout the program
static const size_t chunksize_small = (1 << 6); //12 //6
static const size_t chunksize_medium = (1 << 10); //14 //10
static const size_t chunksize_large = (1 << 11); //14 //11
...
static int choose_list(block_t *block);
...
// Find the previous block in the miniblock free list
static block_t *prev_miniblock(block_t *block) {
  ...
  {
    if(block == curr_block) return prev;
    prev = curr_block;
  }
...
/* Set the 3rd last header and/or footer bit to 1 if the previous block is
 * a mini-block and 0 otherwise
 */
static void set_prev_mini(block_t *block, bool prev_mini) {
  if (block == NULL)
    return;
...
static void remove_from_free(block_t *block) {
  ...
  } else if (prev == NULL && next != NULL) // Case 2: start of list
  {
...
void *malloc(size_t size) {
  ...
  // block uses the last byte of the heap
...
void free(void *bp) {
  ...
  // Check if block previously used the last byte of the heap
 ...
 * Input: void *ptr (pointer to currently allocated (hopefully) block's payload),
...
static block_t *extend_heap(size_t size) {
  ...
  set_prev_alloc(block, last_byte_used);
  last_byte_used = false;
...
static void split_block(block_t *block, size_t asize) {
  ...
  if ((block_size - asize) >= min_block_size) { // need to split block
    ...
    dbg_printf("GET_SIZE(BLOCK) = %lu\n", get_size(block));
    dbg_printf("PREV_MINI(BLOCK) %d\n", get_prev_mini(block));
    dbg_printf("PREV_MINI(BLOCK_NEXT) %d\n", get_prev_mini(block_next));
...
static block_t *find_fit(size_t asize) {
  ...
  while (true) {
    ...
    else if (asize < get_size(block))
    {
      if (!safety_block)
        safety_block = block;
      else if (get_size(safety_block) > get_size(block))
        safety_block = block;
    }
...
bool mm_checkheap(int line) {
  ...
  for (block = heap_start; get_size(block) > 0; block = find_next(block)) {
    ...
    if (prev_alloc != get_prev_alloc(block)) {
      ...
      printf("The 2nd lowest bit (prev_alloc) status is incorrect at block %p\n",
             block);
      return false;
    }
  ...
  for (int i = 0; i < num_lists; i++) {
    ...
    for (block = heads_of_free[i]; block != NULL; block = get_next_ptr(block)) {
      ...
      // Check that next and previous blocks are doubly-linked for non-miniblocks
      if(i != 0)
      {
...
static void write_footer(block_t *block, size_t size, bool alloc) {
  ...
  //if(is_mini(block)) return; //mini-blocks don't get footers!
...
static block_t *find_prev(block_t *block) {
  ...
  if(get_prev_mini(block))
    return (block_t *)((char *)block - dsize); //miniblocks are dsize bytes long
