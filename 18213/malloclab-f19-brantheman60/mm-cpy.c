 ...
 *  free blocks
 *  Miniblocks can be either allocated or freed blocks with only a header and a
 *  byte for the payload or next pointer
 *
 *  Free block:           [ HEADER | PREV | NEXT | PAYLOAD (garbage) | FOOTER ]
 *  Free miniblock:       [ HEADER | PREV ]
 *  Allocated block:      [ HEADER | PAYLOAD ]
 *  Allocated mini block: [ HEADER | PAYLOAD ]
 *
 *  Organization and Manipulation of free list:
 *  This program uses a segregated LIFO free list implementation, so the program
 *  only needs to look at free blocks while allocating and sets a new free block
 *  to the head of its designated list (based on size). To search through the free
 *  list, a best-fit search is used
...
static const size_t chunksize = (1 << 6); //6

// Allow chunksize to change throughout the program
static const size_t chunksize_small = (1 << 6); //6
static const size_t chunksize_medium = (1 << 10); //10
static const size_t chunksize_large = (1 << 11); //11
...
// Get third bit from header, which flags prev block as miniblock(1) or not(0)
static const word_t prev_mini_mask = 0x4; // still need to >> 2
...
static bool is_mini(block_t *block);
...
static void set_prev_alloc(block_t *block, bool prev_alloc) {
  ...
  if (!get_alloc(block) && !is_mini(block)) {
...
static const int num_lists = 9;    // number of doubly-linked free lists (by size)
block_t *heads_of_free[num_lists]; // array of pointers to block_t
static const int SIZE0 = 16;       // max size of block in free list 0
static const int SIZE1 = 32;       // max size of block in free list 1
static const int SIZE2 = 64;       // max size of block in free list 2
static const int SIZE3 = 256;      // max size of block in free list 3
static const int SIZE4 = 1024;     // max size of block in free list 4
static const int SIZE5 = 2048;     // max size of block in free list 5
static const int SIZE6 = 4096;     // max size of block in free list 6
static const int SIZE7 = 16384;    // max size of block in free list 7
...
/** MY OWN FUNCTIONS TO DO MINIBLOCKS **/

// Returns if block is, by definition, a miniblock
static bool is_mini(block_t *block) { return get_size(block) <= 16; }

/*
 * prev_miniblock: returns the previous free miniblock in the list by linearly
 *                 cycling through it
 */
static block_t *prev_miniblock(block_t *block) {
  dbg_requires(is_mini(block));
  block_t *prev = NULL;

  // Cycle through miniblock (last) free list to find prev
  for (block_t *curr_block = heads_of_free[0]; curr_block != NULL;
...
// Determine if the previous block is mini from 3rd last header bit
static bool get_prev_mini(block_t *block) {
  return (block->header & prev_mini_mask) >> 2;
}

/*
 * set_prev_mini: Set the 3rd last header and/or footer bit to 1 if the previous
 *                block is a mini-block and 0 otherwise
 */
...
/*
 * insert_in_free: inserts the block in its free list and makes it the new
...
/*
 * remove_from_free: takes out the block from its free list, then links the
...
static void remove_from_free(block_t *block) {
  ...
  } else if (prev == NULL && next != NULL) // Case 2: start of free list
  {
  ...
  } else // Case 4: somewhere between start and last in list
  {
...
void *malloc(size_t size) {
  ...
  asize = round_up(size + wsize, dsize); // can even be 16!
  ...
  set_prev_mini(block, prev_mini);
  // Try to split the block if too large
  split_block(block, asize);

  // next block contains last byte of heap
...
void free(void *bp) {
  ...
  if (block + get_size(block) - 1 == mem_heap_hi())
...
static block_t *coalesce_block(block_t *block) {
  ...
  } else if (!prev_alloc && next_alloc) // Case 3
  {
    ...
    prev_mini = get_prev_mini(block_prev);
    ...
    set_prev_alloc(block_next, false);
    set_prev_mini(block_prev, prev_mini);
...
bool mm_checkheap(int line) {
  ...
  for (block = heap_start; get_size(block) > 0; block = find_next(block)) {
    ...
    if (prev_alloc != get_prev_alloc(block)) {
      ...
      printf(
          "The 2nd lowest bit (prev_alloc) status is incorrect at block %p\n",
          block);
      return false;
    }

    // Check that prev_mini matches the block header's third lowest bit
    if (prev_mini != get_prev_mini(block)) {
      printf("prev_mini = %d, get_prev_mini = %d\n", prev_mini,
             get_prev_mini(block));
      printf("The 3rd lowest bit (prev_mini) status is incorrect at block %p\n",
             block);
      return false;
    }
    ...
    if (!get_alloc(block)) {
      ...
      // Check that header and footer match for non-miniblocks
      if (!is_mini(block))
  ...
  for (int i = 0; i < num_lists; i++) {
    ...
    for (block = heads_of_free[i]; block != NULL; block = get_next_ptr(block)) {
      ...
      // Check that next and previous blocks are doubly-linked for
      // non-miniblocks
      if (i != 0) {
        next = get_next_ptr(block); // next block in list
...
static void print_heap(int i) {
  ...
  for (block = heap_start; get_size(block) > 0; block = find_next(block)) {
    ...
    printf("PREV_ALLOC STATUS: %d, ", get_prev_alloc(block));
    printf("PREV_MINI STATUS: %d\n", get_prev_mini(block));
  }
...
static block_t *find_prev(block_t *block) {
  ...
  if (get_prev_mini(block))
    return (block_t *)((char *)block - dsize); // miniblocks are dsize bytes
                                               // long
